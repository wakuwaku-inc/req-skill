...
From `$ARGUMENTS`, draft `{{issues}}` and `{{requests}}`. Present to the user:
...
1. Single placeholder of impact in `{{requirements}}` (one screen / one component / one copy change).
...
Workspace staging in fast-track (workspace-aware mode only; entirely skipped in no-workspace mode): stage URLs from the draft as reference candidates. Do NOT auto-stage `{{requirements}}` or `{{DoD}}` decisions — fast-track is for trivial requests where the decision is too obvious to record. If the user-modified draft introduces a new term, stage it as a glossary candidate.

### Step 4C — Active-proposal dialogue (complex branch)

Order: `{{requirements}}` → `{{DoD}}` → `{{verify}}` → metadata fields (`{{issue_urls}}`, `{{designs}}`, `{{references}}`, `{{others}}`, `{{due}}`, `{{due_reasons}}`, `{{notes}}`).

Note: this order differs from `/req` (template order) because `/req-brainstorm`'s value is concentrated in `{{requirements}}`, `{{DoD}}`, `{{verify}}` where active proposals matter most; metadata is filled at the end as a quick sweep.
...
**Proposal patterns** (apply the appropriate one(s) per placeholder):
  ...
  1. **Implementation vs. operational alternative** — apply to `{{requirements}}` when the request describes a recurring user task or a constraint that could be met by either side:
  ...
  2. **Scope alternative** — apply to `{{requirements}}` when the request is open-ended:
     「スコープを次のように切れます。
     - MVP: <最小機能> — <週単位>
     - フル: <全要望> — <月単位>
  ...
  3. **Design / placement alternative** — apply to `{{requirements}}` when UI placement / ordering / layout has tradeoffs:
     「画面/配置の候補:
     - 案A: <…> — <利点/欠点>
     - 案B: <…> — <利点/欠点>
     - 案C: <…> — <利点/欠点>
  ...
  4. **DoD proposal** — apply to `{{DoD}}` always in this branch:
     「完成条件案:
     - [ ] <derived from requirements>
     - [ ] <…>
     追加・削除・修正はありますか？」

  5. **Verify proposal** — apply to `{{verify}}`:
     「確認方法案: <誰が、どうやって、どこで確認するか案>。これで合っていますか？」

  6. **Metadata fields** (`{{issue_urls}}`, `{{designs}}`, `{{references}}`, `{{others}}`, `{{due}}`, `{{due_reasons}}`, `{{notes}}`): plain ask, identical to `/req` Step 3 sub-steps 2, 5–9, 12. No proposals (these are factual collections).
...
- **Reference capture**: URLs given for `{{issue_urls}}`, `{{designs}}`, `{{references}}`, `{{others}}` stage as reference candidates.
