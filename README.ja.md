...
開発依頼は情報が曖昧なまま来ることが多く、実装者が読み直しや確認で工数を溶かす原因になります。このスキルは「課題→要望→要件→デザイン→スケジュール→DoD（依頼者視点 / エンジニア視点）」の順で構造化ヒアリングを強制し、依頼者の抜け漏れと実装者の確認往復を減らします。
...
**fast-track (Step 4S)** — 軽微な依頼（単一画面の変更、DoD が自明、実質的な代替案がない）の場合、AI が依頼内容から全フィールドのドラフトを一度に生成し、確認・修正を求めます。長い順次質問は不要です。

**ブレスト対話 (Step 4C)** — それ以外の場合、AI が `{{requirements}}` → `{{DoD}}`（依頼者視点 / エンジニア視点に分割）→ メタデータの順で対話を進め、各分岐で代替案を理由付きで提示します。対話中の決定事項（採用案 + 棄却案 + 理由）は、ワークスペースの `decisions.md` に staging され将来の参照用に残ります。
...
## /req-lite — 軽微な保守依頼向け要件書

Web 保守の軽微な依頼（文言変更・画像差し替え・コピー編集など）で使うコマンド。顧客の依頼文をそのまま貼り付け、必要なら Excel / PDF / 画像のパスを添えるだけで、ほぼ自動で要件書をドラフトする。明示確認は 関連URL と 希望納期 のみ、その他は最後に1度のバッチ質問で完結する。

### こういうときに使う

- 依頼本文がほぼ自己完結している（例: 「TOPページの「○○」を「□□」に変更してください」）。
- 顧客のメール・Slack・Chatwork スレッドをそのまま貼り付けたい。
- 添付資料 (PDF 仕様書、デザインモック、Excel 顧客リスト等) を要件書に取り込みたい。

### `/req` を使うべきとき

- 複数項目を対話で深掘りしたい場合。
- 提案型の壁打ちをしてから要件にまとめたい場合 → `/req-brainstorm` を使う。
- 既存の要件書を更新したい場合 → `/req --update <path>` を使う。
...
出力: `docs/[req-skill/]requirements/YYYY-MM-DD_<title>.md` (課題 / 要望 / 要件 / 関連URL / 希望納期 / 完成条件 が埋まる)。後から `/req --update` で拡張可能。

### `/req-lite` がやらないこと

- 更新モードなし (`/req --update` を使う)。
- ワークスペース staging なし (用語集・決定・参考資料は自動更新されない。ワークスペースを成長させたい場合は `/req` を使う)。
- 議論が必要な場合の `/req-brainstorm` への自動ハンドオフなし (該当コマンドを案内して終了)。
